...
const ZH = {
  ...
  checkin_partial: (name, done, total, unit, subj)=>`📝 ${name}：今天完成了 ${done}/${total} ${unit}${subj ? `（${subj}）` : ""}。` + (Number(total) - Number(done) > 0 ? ` 还剩 ${Number(total) - Number(done)} 个，鼓励明天完成。` : " 全部完成了！"),
...
async function getParentLang(sb, userId) {
...
// ── PARSERS ──
function parseTargetReply(body) {
...
const SKIP_REASONS = [
  ...
  },
  {
    ...
    reason: "exam prep (different subject)",
    emoji: "📝",
...
function isParentActivationIntent(body) {
  const lower = body.trim().toLowerCase();
  return /activate.*parent|parent updates|weekly parent updates|parent of|i\s*am\s*the\s*parent|i'm\s*the\s*parent/.test(lower);
}
function isChildActivationIntent(body) {
  const lower = body.trim().toLowerCase();
  return /daily check-ins|daily checkins|ready for my daily|child check-ins|child checkins/.test(lower);
}
...
serve(async (req)=>{
  ...
  try {
    ...
    const last8Like = normalizedInbound ? `%${normalizedInbound}` : "%";

    const [{ data: childRows }, { data: membershipRows }] = await Promise.all([
      sb.from("sq_children")
        .select("id, name, parent_id, level, conversation_state, conversation_context, study_days, whatsapp_number")
        .ilike("whatsapp_number", last8Like)
        .limit(20),
      sb.from("sq_memberships")
        .select("user_id, parent_name, preferred_language, parent_phone")
        .ilike("parent_phone", last8Like)
        .limit(20)
    ]);
    ...
    const context = child.conversation_context || {};
    const now = new Date();
    ...
    const { count: msgCount } = await sb.from("whatsapp_conversations").select("id", {
      count: "exact",
      head: true
    }).eq("contact_phone", phone).eq("direction", "inbound").gte("created_at", todayStart);
    ...
    if (state === "setting_target") {
      ...
      await sendRaw(phone, `Your parent now manages the weekly study target in the StudyPulse dashboard. You'll still receive your daily target automatically on study days. 📚`);
      return ok();
    }
    // ══════════════════════════════════════
    // STATE: PARTIAL COUNT
    ...
    if (state === "partial_count") {
      ...
      const targetQ = context.target_quantity || 0;
      const targetUnit = context.target_unit || "question";
      ...
      if (subject) {
        const { data: wt } = await sb.from("sq_weekly_targets").select("remaining_quantity").eq("child_id", child.id).eq("subject_name", subject).eq("week_start", weekStart).single();
        if (wt) {
          await sb.from("sq_weekly_targets").update({
            remaining_quantity: Math.max(0, wt.remaining_quantity - completedCount)
          }).eq("child_id", child.id).eq("subject_name", subject).eq("week_start", weekStart);
        }
      }
      // Reset state
      await sb.from("sq_children").update({
      ...
      if (remaining > 0) {
        await sendRaw(phone, `Nice — ${completedCount} ${targetUnit}s done! 📝\n` + `You have *${remaining} ${targetUnit}s* left. Try to finish them by tomorrow! 💪`);
      } else {
        await sendRaw(phone, `Wait, you actually finished all ${targetQ}! 🎉 That counts as *done*. Updated!`);
        await sb.from("sq_checkins").update({
          status: "yes"
        }).eq("child_id", child.id).eq("checkin_date", today);
      }
    ...
    if (preCheck?.status === "set_target" && isPremium) {
      ...
      if (targets && targets.length > 0) {
        ...
        await sendRaw(phone, `Here is your current daily target from your parent:\n${summary}\n\nIf it needs changing, please ask your parent to update it in the StudyPulse dashboard.`);
      } else {
        await sendRaw(phone, `Your parent has not set this week's target yet. Please ask them to update it in the StudyPulse dashboard. 📚`);
      }
      return ok();
    }
    if (preCheck?.status === "set_target" && !isPremium) {
      await sendRaw(phone, `Your parent can manage targets from the StudyPulse dashboard. For now, just reply *yes* or *no* on check-in days 😊`);
      return ok();
    }
    // ── ALREADY CHECKED IN? Block re-replies ──
    const { data: existingCheckin } = await sb.from("sq_checkins").select("id, status, prompt_sent_at, note, target_quantity").eq("child_id", child.id).eq("checkin_date", today).order("created_at", {
      ascending: false
    }).limit(1).single();
    if (existingCheckin && existingCheckin.status !== "pending") {
        ...
        if (skipInfo) {
          ...
          if (membership?.parent_phone && membership.parent_phone !== phone) {
            ...
            if (parentLang === "zh") {
              ...
              parentMsg = isDistress ? ZH.rest_distress(child.name, reasonZh) : `${skipInfo.emoji} ${ZH.rest_normal(child.name, reasonZh)}`;
            } else {
      ...
      const { count: outboundToday } = await sb.from("whatsapp_conversations").select("id", {
        count: "exact",
        head: true
      }).eq("contact_phone", phone).eq("direction", "outbound").gte("created_at", todayStart);
    ...
    // ── GET TODAY'S TARGET ──
    const { data: todayTargets } = await sb.from("sq_weekly_targets").select("subject_name, daily_quantity, target_unit, remaining_quantity").eq("child_id", child.id).eq("week_start", weekStart);
    ...
    if (!parsed) {
      ...
      const welcomeTargetLine = hasTargets ? todayTargets.map((t)=>`• ${t.subject_name}: ${t.daily_quantity} ${t.target_unit}${t.daily_quantity > 1 ? "s" : ""}`).join("\n") : null;
      await sendRaw(phone, isPremium ? isGreeting ? welcomeTargetLine ? `Hi ${child.name}! 👋 Welcome to StudyPulse.\n\nYour target for today is:\n${welcomeTargetLine}\n\nWhen you're done tonight, just reply *done*, *partially*, or *no*.` : `Hi ${child.name}! 👋 Welcome to StudyPulse.\n\nYour parent will set your study target in the dashboard, and I'll check in with you each study day.` : `Hi ${child.name}! 👋 Welcome to StudyPulse check-ins!\n\nWhen the prompt arrives, reply with:\n✅ *done* — completed today's tasks\n📝 *partially* — did some\n❌ *no* — skipped today\n⚡ *did extra* — went beyond the plan` : isGreeting ? `Hi ${child.name}! 👋 Welcome to StudyPulse.\n\nI'll check in with you every Tue, Thu, and Sun covering all your study days. When I do, just reply *done*, *partially*, or *no*.` : `Hi ${child.name}! 👋 Welcome to StudyPulse check-ins!\n\nI'll message you every Tue, Thu, and Sun. Just reply:\n✅ *done* — finished the target\n📝 *partially* — did some\n❌ *no* — skipped`);
      return ok();
    }
    ...
    // Guardrail: remove any duplicate pending rows for the same child/day.
    // This prevents cron from later marking a stale duplicate as forgot.
    ...
    if (parsed.status === "done" || parsed.status === "did_extra") {
      ...
      if (todayTarget) {
        // Update remaining
        const { data: wt } = await sb.from("sq_weekly_targets").select("remaining_quantity").eq("child_id", child.id).eq("subject_name", todayTarget.subject_name).eq("week_start", weekStart).single();
        if (wt) {
          await sb.from("sq_weekly_targets").update({
            remaining_quantity: Math.max(0, wt.remaining_quantity - checkinTargetQty)
          }).eq("child_id", child.id).eq("subject_name", todayTarget.subject_name).eq("week_start", weekStart);
        }
        await sb.from("sq_checkins").update({
          completed_quantity: checkinTargetQty
        }).eq("child_id", child.id).eq("checkin_date", today);
      }
      const doneMsg = hasTargets ? `✅ ${checkinTargetQty} ${formatUnitLabel(checkinTargetQty, todayTarget.target_unit)} of ${todayTarget.subject_name} — done! Great work, ${child.name}! 💪` : `✅ Nice work, ${child.name}! Keep it up 💪`;
      const extraMsg = parsed.status === "did_extra" ? `\n⚡ Extra effort today — respect!` : "";
      await sendRaw(phone, doneMsg + extraMsg);
      if (membership?.parent_phone && membership.parent_phone !== phone) {
        let parentMsg;
        if (parentLang === "zh") {
          parentMsg = hasTargets ? ZH.checkin_done(child.name, todayTarget.subject_name, String(checkinTargetQty), todayTarget.target_unit) : ZH.checkin_done_generic(child.name);
...
async function updateStreak(sb, childId) {
  ...
  if (milestones[streak]) {
    const { data: child } = await sb.from("sq_children").select("name, parent_id").eq("id", childId).single();
    if (child) {
...
async function handleParentReply(sb, membership, action, phone, lang = "en") {
  const { data: children } = await sb.from("sq_children").select("id, name").eq("parent_id", membership.user_id);
  ...
  if (action === "parent_confirm") {
    ...
    const msg = lang === "zh" ? ZH.parent_confirm_adjust(names) : `Got it — we've updated ${names}'s record. Thanks!`;
    await sendRaw(phone, msg);
  }
}
