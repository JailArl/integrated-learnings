...
function isParentActivationIntent(body: string): boolean {
  const lower = body.trim().toLowerCase();
...
function isChildActivationIntent(body: string): boolean {
  const lower = body.trim().toLowerCase();
...
serve(async (req) => {
  ...
  try {
    ...
    if (phoneMembership && isParentActivationIntent(body)) {
      ...
      const ack = pLang === "zh"
        ? `✅ ${parentName}，家长周报已启用。之后我们会把每周总结发到这个号码。\n\n如果要启用孩子每日打卡，请让孩子用自己的手机发送孩子激活链接。`
        : `✅ Parent updates activated, ${parentName}. We'll send weekly summaries to this WhatsApp number.\n\nFor child daily check-ins, your child must send the child activation message from their own phone.`;
