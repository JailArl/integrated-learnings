...
-- Drop existing policies safely (only if they exist)
DO $$ BEGIN
  DROP POLICY IF EXISTS "Users can view their own parent profile" ON parent_profiles;
  DROP POLICY IF EXISTS "Users can insert their own parent profile" ON parent_profiles;
  DROP POLICY IF EXISTS "Users can update their own parent profile" ON parent_profiles;
  ...
  DROP POLICY IF EXISTS "Admin can view all tutor profiles" ON tutor_profiles;
  
  DROP POLICY IF EXISTS "Tutors can view their own certificates" ON tutor_certificates;
  DROP POLICY IF EXISTS "Tutors can insert their own certificates" ON tutor_certificates;
  DROP POLICY IF EXISTS "Admin can view all certificates" ON tutor_certificates;
  
  DROP POLICY IF EXISTS "Parents can view their own requests" ON parent_requests;
  DROP POLICY IF EXISTS "Parents can insert their own requests" ON parent_requests;
  DROP POLICY IF EXISTS "Tutors can view requests that are open" ON parent_requests;
  DROP POLICY IF EXISTS "Admin can view all requests" ON parent_requests;
  
  DROP POLICY IF EXISTS "Tutors can view their own bids" ON tutor_bids;
  DROP POLICY IF EXISTS "Tutors can insert their own bids" ON tutor_bids;
  DROP POLICY IF EXISTS "Admin can view all bids" ON tutor_bids;
  
  DROP POLICY IF EXISTS "Parents can view their matches" ON matches;
  DROP POLICY IF EXISTS "Tutors can view their matches" ON matches;
  DROP POLICY IF EXISTS "Admin can view all matches" ON matches;

  DROP POLICY IF EXISTS "Tutors can submit their own appeals" ON ai_interview_appeals;
  DROP POLICY IF EXISTS "Admin can view all appeals" ON ai_interview_appeals;
END $$;
